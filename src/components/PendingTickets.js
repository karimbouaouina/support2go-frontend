  ...
  useEffect(() => {
    const fetchTickets = async () => {
      try {
        ...
        const pendingTickets = response.data.filter(ticket => ticket.status === 'Pending');
    ...
    <><div className="bg-blue-50 min-h-screen">
      ...
      <main className="max-w-7xl mx-auto sm:px-6 lg:px-8 py-12">
        <div className="bg-white overflow-hidden shadow rounded-lg">
          ...
          <h3 className="text-lg leading-6 font-medium text-gray-900">Pending Tickets</h3>
            <div className="mt-4">
              <div className="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  {tickets.map((ticket) => (
                    ...
                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${ticket.status === 'Pending' ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800'}`}>
                    {ticket.status}
                    </span>
                    </td>
